...
// Use mock data when explicitly enabled or when no API URL is configured
const USE_MOCK =
...
let MOCK = [
  ...
  mapEvent({
    ...
    capacity: 200,
    attendees: [{ email: "omar@roost.app", status: "going" }],
    visibility: "public",
    status: "scheduled",
  }),
  mapEvent({
...
// --- Manage attendees (detail panel convenience) -----------------------------
export async function updateAttendeeStatus(eventId, email, status) {
  if (USE_MOCK) {
    await delay();
    const ev = MOCK.find((e) => e._id === eventId || e.id === eventId);
    if (!ev) throw new Error("Event not found");
    const idx = (ev.attendees || []).findIndex((a) => a.email === email);
    if (idx >= 0) ev.attendees[idx].status = status;
    else ev.attendees.push({ email, name: email.split("@")[0], status });
    return mapEvent(ev);
  }
  const res = await api.put(`/events/${eventId}/attendees`, { email, status });
  return mapEvent(res.data);
}
